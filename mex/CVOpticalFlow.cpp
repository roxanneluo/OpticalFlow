...
// flow_in = flow_out*scale + shift
// flow_out = (flow_in - shift) / scale
cv::Mat CVOpticalFlow::writeFlow(const cv::Mat& flow_in,
                                 double *scale, double *shift,
                                 bool enable_flip_sign) {
  double min_disp = 1e9, max_disp = -1e9;
  for (int i = 0; i < flow_in.rows; ++i) {
    for (int j = 0; j < flow_in.cols; ++j) {
      double d =flow_in.at<double>(i, j); 
      min_disp = min(min_disp, d);
      max_disp = max(max_disp, d);
    }
  }

  bool flip_sign = enable_flip_sign && (max_disp <= 0);
  *shift = !flip_sign? min_disp:-max_disp;
  unsigned char MAX_V = 255;
  *scale = (max_disp - min_disp) / double(MAX_V); 
  double bias = flip_sign? max_disp:*shift;
  double div = flip_sign? -*scale:*scale;
  
  Mat result(flow_in.rows, flow_in.cols, CV_8U);
  for (int i = 0; i < result.rows; ++i)
    for (int j = 0; j < result.cols; ++j) {
      double v = (flow_in.at<double>(i, j) - bias) / div;
      result.at<unsigned char>(i, j) = min(MAX_V, static_cast<unsigned char>(v + 0.5));
    }
  return result;
}
