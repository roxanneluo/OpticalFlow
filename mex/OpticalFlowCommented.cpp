																    ...
																    double alpha, int nOuterFPIterations, int nInnerFPIterations, int nSORIterations)
{
    ...
    // imdx = d/dx (0.4 * Blur(im1) + 0.6 * Blur(im2))
    // imdx = d/dy ditto
    // imdt = Blur(im2) - Blur(im1)
    ...
    for(int hh=0;hh<nInnerFPIterations;hh++) {
      ...
      Phi_1st.reset(); _FlowPrecision* phiData=Phi_1st.data();
      double temp;
      ...
      for(int i=0;i<nPixels;i++) { // Regularization: Delta W 
      ...
      // Output to foo1, input u, weight Phi_1st
