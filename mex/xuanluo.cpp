...
void write_flow(const Mat &flow, const string& flow_filename,
                const string &spec_filename,
                bool enable_flip_sign) {
  double scale, shift;
  Mat flow_to_write = CVOpticalFlow::writeFlow(flow, &scale, &shift, enable_flip_sign);
  cv::imwrite(flow_filename.c_str(), flow_to_write);

  // dump specs
  std::ofstream fout(spec_filename.c_str());
  fout << scale << std::endl << shift << std::endl;
  fout.close();
  //printf("disparity x scale = %f, shift = %f\n", scale, shift);
...
int main(int argc, char ** argv) {
  cv::CommandLineParser parser(
      ...
      "{@vx_filename||}{@vx_spec_filename||}"
      "{@vy_filename||}{@vy_spec_filename||}"
      );
  if (parser.has("help")) {
    parser.printMessage();
    return 0;
  }
  string im1_filename = parser.get<string>("@image1_filename");
  string im2_filename = parser.get<string>("@image2_filename");
  string vx_filename = parser.get<string>("@vx_filename");
  string vx_spec_filename = parser.get<string>("@vx_spec_filename");
  string vy_filename = parser.get<string>("@vy_filename");
  string vy_spec_filename = parser.get<string>("@vy_spec_filename");
  // param
  ...
  printf("standard\n");
  Mat im1 = imread(im1_filename.c_str());
  Mat im2 = imread(im2_filename.c_str());
  ...
  imshow("vy", vy);
  Mat diff;
  cv::absdiff(mywarp, im1, diff);
  imshow("diff", diff);
  waitKey(0);

  mywarp.convertTo(mywarp, CV_8UC3, 255, 0);
  cv::imwrite("warp.png", mywarp);
  */
  
  write_flow(vx, vx_filename, vx_spec_filename, true);
  if (!vy_filename.empty())
    write_flow(vy, vy_filename, vy_spec_filename, false);
}
