...
public:
  ...
  virtual bool fromMat(cv::Mat &im);
  virtual bool fromMats(vector<cv::Mat> &im);
  virtual void toMat(cv::Mat &im);
...
template <class T> bool Image<T>::fromMats(vector<cv::Mat> &im) {
  clear();
  ...
  if (im[0].type() == CV_64F) {
    //printf("%d, %d\n", (int)im.step, im.cols*3);
    //cv::Mat reorder;
    //cv::cvtColor(im, reorder, CV_BGR2RGB);
    printf("Width = %d, Height = %d, Channels = %d\n", imWidth, imHeight, nChannels);
    for (int k = 0; k < im.size(); k++) {
      for (int i = 0; i < imHeight; i++) {
        int offset1 = i * imWidth * nChannels;
        for (int j = 0; j < imWidth; j++) {
          pData[offset1 + j * nChannels + k] = im[k].at<double>(i, j);
        }
      }
    }
  } else {
    printf("only support 64F\n");
  }
  computeDimension();
  return true;
}

template <class T> bool Image<T>::fromMat(cv::Mat &im) {
  clear();
  if(im.data == NULL) 
    return false;
  if(im.type()!= CV_8UC1 && im.type()!=CV_8UC3 && im.type()!=CV_64FC3 && im.type()!=CV_64F) {
    printf("Error: unsupported type\n");
    return false;
  }

  imWidth = im.size().width;
  imHeight = im.size().height;
  ...
  if(im.type()== CV_8UC1 || im.type()==CV_8UC3) {
    for(int i=0;i<imHeight;i++) {
      ...
      for(int j=0;j<im.step;j++) {
        pData[offset1+j] = (T)im.data[offset2+j]/255;
      }
    }
  } else {
    //printf("%d, %d\n", (int)im.step, im.cols*3);
    ...
    //cv::cvtColor(im, reorder, CV_BGR2RGB);
    printf("Width = %d, Height = %d, Channels = %d\n", imWidth, imHeight, nChannels);
    memcpy(pData, im.data, sizeof(double) * im.rows * im.cols * nChannels);
  }
  computeDimension();
  colorType = BGR; 
  return true;
...
template <class T> void Image<T>::toMat(cv::Mat &im) {
  int width = imWidth;
  int height = imHeight;
  int nchannels = nChannels;
  const T* pImagePlane = pData;

	ImageIO::ImageType imtype;
	if(IsDerivativeImage) {
		imtype = ImageIO::derivative;
  } else {
		imtype = ImageIO::standard;
  }
	...
	switch(imtype){
		case ImageIO::standard:
			break;
		case ImageIO::derivative:
	...
	if(typeid(T) == typeid(unsigned char) && imtype == ImageIO::standard) {
		for(int i = 0;i<height;i++)
																		...
																		const Image<T2>& vx,const Image<T2>& vy) const
{
	...
	for(int i  = 0; i<height; i++)
		for(int j = 0;j<width;j++)
		{
			...
			double x = j + vx.pData[offset];
			double y = i + vy.pData[offset];
      //printf("%f %f %d\n", vx.pData[offset], vy.pData[offset], sizeof(vx.pData[offset]));
